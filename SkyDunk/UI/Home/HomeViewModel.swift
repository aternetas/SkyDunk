...
class HomeViewModel: BaseViewModel {
    ...
    private func getGames() {
        DispatchQueue.global().asyncAfter(deadline: .now() + 0.5, execute: { [weak self] in
            self?.games = [
                Game(homeTeam: .bostonCeltics, guestTeam: .atlantaHawks, gameDate: Date(), homeScore: 0, guestScore: 0, bets: [Bet(description: "fias satao fit xiiu aiobx oteswy te qotn ydcn innie3 tktk", date: Date(), amount: 10, coefficient: 2, teams: [TeamType.bostonCeltics, TeamType.atlantaHawks], isSuccess: nil)]),
